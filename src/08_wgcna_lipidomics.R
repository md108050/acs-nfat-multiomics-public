...
# 出力先
out_dir_figs   <- "results/figures/wgcna_lipidomics"
out_dir_tables <- "results/tables/wgcna_lipidomics"
...
df_sphingo             <- read_csv("data/processed/lipidomics/norm_Sphingolipids.csv", show_col_types = FALSE)

# 2. リピドミクスと共通のサンプルに絞り込む ----
common_samples <- df_sphingo |>
  filter(!is.na(RNAseq_Sample_ID)) |>
  pull(Research_ID) |>
  intersect(rownames(meta))

meta_wgcna   <- meta[common_samples, , drop = FALSE]
counts_wgcna <- counts_batch_corrected[, common_samples]

# 3. 共通サンプルのみでDESeq2を実行 ----
...
# 6. Traits データの作成（Cer(d18:1/16:0) のみ） ----
datTraits <- df_sphingo |>
  filter(Research_ID %in% rownames(datExpr)) |>
  dplyr::select(Research_ID, Cer_d18_1_16_0) |>
...
net <- blockwiseModules(
  ...
  saveTOMs          = TRUE,
  saveTOMFileBase   = "data/processed/rnaseq/TOM_lipidomics",
  verbose           = 3
)

saveRDS(net,       "data/processed/rnaseq/wgcna_lipidomics_net.rds")
saveRDS(datExpr,   "data/processed/rnaseq/wgcna_lipidomics_datExpr.rds")
saveRDS(datTraits, "data/processed/rnaseq/wgcna_lipidomics_datTraits.rds")
...
# モジュール遺伝子一覧の保存
module_df_lipid <- data.frame(
  gene   = names(net$colors),
  colors = labels2colors(net$colors)
)
write_csv(module_df_lipid, file.path(out_dir_tables, "module_genes.csv"))

for (mod_color in unique(module_df_lipid$colors)) {
  module_df_lipid |>
    filter(colors == mod_color) |>
    write_csv(file.path(out_dir_tables, paste0("module_", mod_color, ".csv")))
}

# 10. モジュールとリピドミクスの相関ヒートマップ ----
...
pdf(file.path(out_dir_figs, "module_trait_heatmap.pdf"), width = 10, height = 6)
par(mar = c(6, 8.5, 3, 3))
labeledHeatmap(
  ...
  cex.text      = 1,
  zlim          = c(-1, 1),
  main          = "Module-Sphingolipid relationships"
)
dev.off()

# 11. モジュールのORA (enrichR) ----
dbs     <- c("GO_Biological_Process_2025", "MSigDB_Hallmark_2020", "NCI-Nature_2016")
modules <- unique(module_df_lipid$colors)

enrichr_results_lipid <- list()

for (mod_color in modules) {
  genes <- module_df_lipid |>
    filter(colors == mod_color) |>
    pull(gene)
  ...
  enrichr_results_lipid[[mod_color]] <- bind_rows(mod_results) |>
    mutate(
...
selected_terms <- enrichr_results_lipid[[plot_module]] |>
...
p_ora <- ggplot(selected_terms, aes(x = Term, y = logp, color = logp)) +
  ...
  scale_color_gradient(low = "#6A5ACD", high = "#4B0082") +
...
ggsave(file.path(out_dir_figs, "ora_enrichr_yellow.pdf"), p_ora, width = 18, height = 12, units = "cm")
ggsave(file.path(out_dir_figs, "ora_enrichr_yellow.png"), p_ora, width = 18, height = 12, units = "cm", dpi = 300)
